...
<div class="content"> 
  ...
  <div class="box1">
    ...
    <div class="box">    
      <div class="breadcrumb">Volume By Agent</div>
      <div id="right2"> 
        <!-- table -->
        <div class="box1"> 
          <!-- display box / first -->
                ...
                <?php 
                	...
                	foreach ($isos as $iso) { 
                		foreach($statusarray as $status){
                			if($status==2){
                				$preapp = $common->getAgentVolume($iso['User']['id'], 2, $index );
                				$preappTotal += $preapp;
                			}elseif($status==3){
                				$incomplete = $common->getAgentVolume($iso['User']['id'], 3, $index);
                				$incompleteTotal += $incomplete;
                			}elseif($status==4){
                				$pending = $common->getAgentVolume($iso['User']['id'], 4, $index );
                				$pendingTotal += $pending;
                			}elseif($status==5){
                				$processing = $common->getAgentVolume($iso['User']['id'], 5, $index );
                				$processingTotal += $processing;
                			}elseif($status==9){
                				$integrating = $common->getAgentVolume($iso['User']['id'], 9, $index );
          ...
          <!-- display box / first --> 
           ...
           <!-- display box / first -->
           <div class="breadcrumb">Active Accounts By Agent</div>
          <div class="display_row padding_top_zero">
            <div class="table">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>

...
//				$MerchantRec = $this->Merchant->find('count',array('conditions' => array($condition)));			
			...
			if(isset($this->request->query['Submit']) && $this->request->query['Submit']=='Export'){
				...
				$header_row = array("COMPANY NAME",'URL',"CONTACT NAME","CONTACT EMAIL","CONTACT PHONE","PROCESSOR","STATUS","MANAGER","ISO","AGENT","REFERRAL","PROCESSING VOLUME");
				...
				foreach($MerchantRec as $result)
				{
					...
					$this->request->data['Merchant']['volume'] = $result['Merchant']['volume'];
					$row = array(
							...
							$result['ISO']['user_Name'],
							$result['Agent']['user_Name'],
...
PER ITEM % has set to '.$this->request->data['Rate'][$key].'$<br>';
	...
	public function superadmin_merchantaction($merchantID = null) {
			...
			if ($this->request->is('get')) {
				...
				if($this->request->data['Submit']=='Send Email'){
						$managerID = $this->request->data['Processor']['managerID'];
						$agentID = $this->request->data['Processor']['agentID'];
						$sendto = $this->request->data['Processor']['sendTo'];
						$sendfrom = $this->request->data['Processor']['sendFrom'];
						//$sendfrom = "jason@total-apps.com";
						$subject = "Your Online Application Link - ".$this->request->data['Processor']['legalName'];
						$bodyText = "";
						...
						$list = array();	
						$cclist = array();
						if($this->request->data['Processor']['manager'] && $this->request->data['Processor']['manager']!=1)
						{
							$list[]=$this->request->data['Processor']['manager'];
						}
						if($this->request->data['Processor']['iso'] && $this->request->data['Processor']['iso']!=1)
						{
							$list[]=$this->request->data['Processor']['iso'];
						}
						if($this->request->data['Processor']['Agent'] && $this->request->data['Processor']['Agent']!=1)
						{
							$list[]=$this->request->data['Processor']['Agent'];
						}
						if($this->request->data['Processor']['referral'] && $this->request->data['Processor']['referral']!=1)
						{
							$list[]=$this->request->data['Processor']['referral'];
						}
						if($this->request->data['Processor']['cc_email']!=""){
							$cclist = explode(",",trim($this->request->data['Processor']['cc_email']));
						}
						if($this->request->data['Processor']['bcc_email']!=""){
							$bcclist = explode(",",trim($this->request->data['Processor']['bcc_email']));
						}
						...
						if($managerID!=''){
							$managerEmail = $this->request->data['Processor']['managerEmail'];
							$managerName = $this->request->data['Processor']['managerName'];
							$managerCompany = $this->request->data['Processor']['managerCompany'];
							$managerCompanyUrl = $this->request->data['Processor']['managerCompanyUrl'];
							$managerPhone = "Phone:".$this->request->data['Processor']['managerPhone'];
						}else{
							$managerEmail = '';
							$managerName = '';
							$managerCompany = '';
						...
						$merchantContactName = $this->request->data['Processor']['merchantContactName'];
						$merchantLegalName = $this->request->data['Processor']['legalName'];
						...
						$appFrom = $this->request->data['Processor']['appFrom'];
						...
						if($package=='1' && file_exists($attachfilePath)){
							...
							$bodyText .= "Find Your Application document in attachment to complete your application on Total-Apps";
							...
							$processr_id=$this->request->data['processor_id'];
						...
						if(!empty($processr_id) && $processr_id=='1357'){
							...
							if(stripos($bodyText,"[PREAPP REQUEST URL]")===false && stripos($bodyText,"[MERCHANT/CONTACT NAME]")===false){
								$bodyText = "Dear ".$merchantContactName.",<br/><br/>You are just a few, short steps away from being able to accept credit cards. Getting your merchant account setup properly and efficiently is our number one goal.<br/><br/>In order to process your application as quickly as possible, we offer a fully automated, online application system. Please click on the link below to start your online application:<br/><br/>".$preapp_req_url."<br/><br/>".$bodyText;								
							}else{
	...
	public function superadmin_merchantcommission($merchantID	 = null) {
		...
		if ($this->request->is('get')) {
			...
			if(!is_array($commissionData)){
				$this->request->data['Commission']['agent_split']		=	$agentRec['User']['agent_split'];
				$this->request->data['Commission']['referral_split']	=	$agentRec['User']['referral_split'];
				$this->request->data['Commission']['iso_split']			=	$agentRec['User']['iso_split'];
			}
			//====================== End Pushkar =================== 
	...
	public function superadmin_processinghistory($merchantID = null) {
		...
		if ($this->request->is('get')) {
			...
			$this->loadModel('Commissionlog');
			$log = $this->Commissionlog->find("all",array('conditions'=>array('Commissionlog.merchantID'=>$merchantID),'order'=>array('Commissionlog.date DESC'),'group' => array('Commissionlog.MID','Commissionlog.amount_processed','Commissionlog.gross_revenue','Commissionlog.totalapps_income','Commissionlog.date')));
    ...
    */
    public function superadmin_approved_contract_ex_date($merchantId = null){
    	if($merchantId != null){
    		...
    		$timestamp = CURRENTDATE;
    		$noteData['Merchantnote']['timeStamp'] = $timestamp;
    		$noteData['Merchantnote']['note'] = "Merchant Application is approved! Contract Expiration date is ".date('m/d/Y',strtotime($datePlusThreeYears));
    		$this->Merchantnote->save($noteData);

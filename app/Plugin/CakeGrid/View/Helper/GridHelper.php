...
class GridHelper extends AppHelper {
 ...
 var $components = array('Session');
	...
	function addColumn($title, $valuePath, array $options = array(),$actionName=''){
	...
	private function __generateColumn($result, $column){
		...
		$ColumnValuePath=$column['valuePath'];
		
		if(strpos($ColumnValuePath,'start_date') || strpos($ColumnValuePath,'end_date'))
		{
			$value = Set::extract($column['valuePath'], $result);
			$value = array_pop($value);
			$value=date('m-d-Y  h:m:i A',$value);
			
		}elseif(strpos($ColumnValuePath,'online_since'))
		{
			$value = Set::extract($column['valuePath'], $result);
			$value = array_pop($value);
			$value=date('m-d-Y  h:m:i A',$value);
			
		}elseif(strpos($ColumnValuePath,'created'))
		{
			$value = Set::extract($column['valuePath'], $result);
			$value = array_pop($value);
		...
		}elseif(strpos($ColumnValuePath,'active'))
		{
			$value = Set::extract($column['valuePath'], $result);
		...
		}elseif(strpos($ColumnValuePath,'message'))
		{
			$value = Set::extract($column['message'], $result);
			if(is_array($value))
			{
				$value="HTML..";
				//$value=substr(strip_tags($value['EmailTemplate']['message']),0.200).'...';
			}else
			{
				$value="HTML..";
				//$value=substr(strip_tags($value['EmailTemplate']['message']),0.200).'...';
			}
		}elseif(strpos($ColumnValuePath,'volume'))
		{
			$value = Set::extract($column['volume'], $result);
			App::uses('CakeNumber', 'Utility');
			$value = CakeNumber::currency($value['Merchant']['volume'], 'USD');			
			
		}elseif(strpos($ColumnValuePath,'merchantDBA'))
		{
			$value = Set::extract($column['merchantDBA'], $result);
			$value=substr($value['Merchant']['merchantDBA'],0,15).'...';
			//App::uses('CakeNumber', 'Utility');
			//$value = CakeNumber::currency($value['Merchant']['volume'], 'USD');			
			
		}elseif(strpos($ColumnValuePath,'merchantStatus'))
		{
		
			
			$value = Set::extract($column['merchantStatus'], $result);
			
			$test=explode('_',$value['Merchant']['merchantStatus']);
			
			$this->loadModel('Merchantpipeline');
			APP::import('Model','Merchantpipeline');
			$this->Merchantpipeline = new Merchantpipeline();
			
			$Rec=$this->Merchantpipeline->find('first',array('recursive'=>-1,'order by'=>'desc','conditions'=>'Merchantpipeline.merchantID="'.$test[0].'" and Merchantpipeline.statusTo="'.$test[1].'"'));
		//	pr($Rec['Merchantpipeline']['statusToDate']);
		
		$currentdate=date('Y-m-d',time());
			
		if($Rec['Merchantpipeline']['statusToDate'])
		{
		$diff=time()-strtotime($Rec['Merchantpipeline']['statusToDate']);
			
			$years = floor($diff / (365*60*60*24));
			$months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
			$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
			//pr($Rec);die;
			if($days==0 && $months==0 && $year==0)
			{
				$value='Today';
			}else
			{
		...
		}else
		{
			...
			//$value = $Rec['Merchantpipeline']['statusToDate'].'______'.$currentdate;
			
				//$value=substr(strip_tags($value['EmailTemplate']['message']),0.200).'...';
			
		}elseif(strpos($ColumnValuePath,'sendMailStatus'))
		{
			$value = Set::extract($column['valuePath'], $result);
			if(array_pop($value))
			{
		...
		}elseif(strpos($ColumnValuePath,'user_group_id'))
		{
		
			$value = Set::extract($column['valuePath'], $result);
			$GroupIds=explode(',',$value['0']);
			
			APP::import('Model','UserGroup');
			$this->UserGroup = new UserGroup();
			
			$data='';
			foreach($GroupIds as $values)
			{
				 $name=$this->UserGroup->find("first",array('fields'=>'name','conditions'=>'UserGroup.id="'.$values.'"','recursive'=>-1)); 
				 $data.=$name['UserGroup']['name'].' , ';
				//echo $common->Groupname($values).' , ';
			}
			$value[0]=$data;
			$value = array_pop($value);
		
		}elseif(strpos($ColumnValuePath,'file_path'))
		{
			
			
			$value = Set::extract($column['valuePath'], $result);
			$values=FULL_BASE_URL.router::url('/',false).'folders/'.$userId.'/'.$value[0];
			$valuess[0]=$value[0];
			if(array_pop($value))
			{
				$value = '<!--<a href="'.$values.'" target="_blank">Download</a><br>-->
				 <a class="example8" id="example8" href="#inline_example1" onclick=popup("'.$valuess[0].'")>View</a>
				';
		...
		if(isset($column['options']['element']) && $column['options']['element'] != false){
			...
			if(isset($column['options']['type']) && $column['options']['type'] == 'date'){
			...
			}  else if(isset($column['options']['type']) && $column['options']['type'] == 'url'){

				$urlValue = $value;

				// Check whether url value contains http:// or not
				...
				// check if URL legnth greater than 22 or not
				if(strlen($value) >18){
				$value=substr($value,0,18).'..';}
				
				$value = $this->Html->link($value,$urlValue,array('target'=>'_blank'));
